  ...
  function getOneProduct(id) {
   ...
   useEffect(() => {
     ...
     function handleChangeUpdate(evt) {
      const value = evt.target.value;
      if (evt.target.name === "price1") {
      setUpdateProduct({ ...updateProduct, price: value });
      }
      } 

      function handleOnSubmitUpdate(e) {
        e.preventDefault();
        console.log(e.target.value);
...
<input type="text" id="message" name="message" placeholder="id" onChange={(e) =>getOneProductUpdate(e.target.value)} />
{viewer3 && <div>Product: {showOneItem}</div>}
<form action="">
<input type="number" placeholder="price1?" name="price1" value={updateProduct.price} onChange={handleChangeUpdate} />
<button type="submit" onClick={handleOnSubmitUpdate}>submit</button> 
</form>
</div>
</div>
);
